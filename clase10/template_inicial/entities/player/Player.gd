...
func _ready():
	...
	self.connect("ice_started", camara, "on_ice_started")
	self.connect("ice_stopped", camara, "on_ice_stopped")
...
func _handle_cannon_actions(should_fire:bool = true):
	...
	if Input.is_action_just_pressed("fire_cannon") and is_firing:
		emit_signal("ice_started")
	elif Input.is_action_just_released("fire_cannon"):
		emit_signal("ice_stop")
